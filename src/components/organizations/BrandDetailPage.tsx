...
export function BrandDetailPage({
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-none ring-0 transition-all ">
        <CardContent className="p-6">
          <div className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
            <div className="flex items-start gap-4">
              ...
              <div>
                <h2 className="text-xl md:text-2xl font-semibold text-slate-950">{brand.name}</h2>
                <a
      ...
      <div className="grid gap-6 xl:grid-cols-[1.05fr_1fr]">
        <InfoCard title="Brand Profile">
          ...
          <InfoRow icon={<CalendarDays className="h-4 w-4" />} label="Created" value={formatDisplayDate(brand.createdAt.toISOString())}/>
          <InfoRow
            icon={<CalendarDays className="h-4 w-4" />}
            label="Account Deleted At"
value={
  brand.deletedAt
    ? formatDisplayDate(brand.deletedAt.toISOString())
                  ...
                  <td className="px-4 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      {typeof item.change === 'number' && (
                        <span className="inline-flex rounded-full bg-orange-100 px-3 py-1 text-xs font-semibold text-blue-700">
...
function StatCard({
  ...
  return (
    <Card className="rounded-2xl border border-slate-200 bg-white shadow-none ring-0 transition-all">
      <CardContent className="flex items-start justify-between p-5">
        <div>
          <p className="text-sm font-medium text-slate-500">{label}</p>
          <p className="mt-2 text-xl md:text-2xl  font-semibold tracking-tight text-slate-950">
            {value}
          </p>
        </div>
        <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-orange-50 text-blue-600">
          {icon}
        </div>
      </CardContent>
    </Card>
